...
-module(erlcloud_kinesis_firehose).

%%% Library initialization.
-export([configure/2, configure/3, configure/4,  new/2, new/3]).

-export([
  %create_stream/2, create_stream/3,
  %delete_stream/1, delete_stream/2,
  list_delivery_streams/0, list_delivery_streams/1, list_delivery_streams/2, list_delivery_streams/3,
...
%%
%% `
%% erlcloud_kinesis:list_delivery_streams().
...
list_delivery_streams(Config) when is_record(Config, aws_config) ->
   erlcloud_kinesis_firehose_impl:request(Config, "Firehose_20150804.ListDeliveryStreams", []);
list_delivery_streams(ExclusiveStartStreamName) ->
   Json = [{<<"ExclusiveStartDeliveryStreamName">>, ExclusiveStartStreamName}],
   erlcloud_kinesis_firehose_impl:request(default_config(), "Firehose_20150804.ListDeliveryStreams", Json).
...
list_delivery_streams(ExclusiveStartStreamName, Config) when is_record(Config, aws_config) ->
   Json = [{<<"ExclusiveStartDeliveryStreamName">>, ExclusiveStartStreamName}],
   erlcloud_kinesis_firehose_impl:request(Config, "Firehose_20150804.ListDeliveryStreams", Json);
...
put_record(StreamName, Data) ->
  Json = [{<<"DeliveryStreamName">>, StreamName}, {<<"Record">>, [{<<"Data">>, base64:encode(Data)}]}],
  erlcloud_kinesis_firehose_impl:request(default_config(), "Firehose_20150804.PutRecord", Json).
